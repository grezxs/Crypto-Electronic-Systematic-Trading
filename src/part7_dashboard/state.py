...
The bot (``python main.py``) and the web terminal (``frontend/index.html``,
served by ``scripts/serve_frontend.py`` or Vercel via ``api/state.py``) are
SEPARATE processes, so they share state through a small JSON snapshot file (no
Redis needed). ``StatePublisher`` registers as the
