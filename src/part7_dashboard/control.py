...
The web terminal and the bot are separate processes, so the terminal's
buttons can't call the bot directly. Instead they POST to ``/api/control``
(``scripts/serve_frontend.py`` / ``api/control.py``), which writes a tiny JSON
command file (``runtime/control.json``) that the bot polls every MARKET tick:
