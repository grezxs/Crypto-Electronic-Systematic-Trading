...
class StrategyManager:
    ...
    def on_market(self, event: MarketEvent) -> Optional[list[SignalEvent]]:
        ...
        # stamp the signal with the MARKET event's time, not wall-clock. Otherwise
        # SignalEvent.ts defaults to time.time(), which breaks downstream time
        # gates (e.g. RiskManager cooldown) in a backtest, where a full day of
        # bars replays within one wall-clock second and the cooldown never expires.
        signal.ts = now
