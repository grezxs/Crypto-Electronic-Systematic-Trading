...
def _ccxt_history(symbol: str, n_bars: int, timeframe: str, tf_sec: int,
    ...
    try:
        if timeframe in ex.timeframes:
            candles = _paginate_candles(ex, symbol, timeframe, tf_sec, n_bars)
        else:
            # Binance has no native sub-minute bar except 1s (no 10s/30s). Build
            # the requested bar by pulling 1s klines and aggregating `tf_sec` of
            # them into one candle — lets the backtest replay the live bot's ~10s
            # sampling cadence instead of only 1m closes.
            base = _paginate_candles(ex, symbol, "1s", 1, n_bars * tf_sec)
            candles = _resample(base, tf_sec)[-n_bars:]
            log.info("resampled %d 1s bars -> %d %s bars", len(base), len(candles), timeframe)
    finally:
        try:
            ex.close()
        except Exception:
...
def _paginate_candles(ex, symbol: str, timeframe: str, tf_sec: int,
                      n_bars: int) -> list[Candle]:
    """Page ``fetch_ohlcv`` back ``n_bars`` bars, de-dup, oldest first."""
    ...
    return [Candle(ts=r[0] / 1000.0, open=float(r[1]), high=float(r[2]),
                   low=float(r[3]), close=float(r[4]), volume=float(r[5]))
            for r in ordered]


def _resample(base: list[Candle], group: int) -> list[Candle]:
    """Aggregate every ``group`` consecutive candles into one (OHLCV)."""
    out: list[Candle] = []
    for i in range(0, len(base) - group + 1, group):
        chunk = base[i:i + group]
