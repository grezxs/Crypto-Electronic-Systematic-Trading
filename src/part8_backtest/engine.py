...
def run_backtest(
    ...
    *,
    ...
    # orders decided on bar i fill at bar i+1's OPEN — a signal computed from a
    # bar's close cannot execute until the next bar actually trades. Filling on
    # the same bar's close would be lookahead (knowing the close while acting on it).
    pending: list[tuple[Order, str]] = []
    for i, c in enumerate(candles):
        # settle the previous bar's orders at THIS bar's open before anything else
        for order, reason in pending:
            _execute(order, c.open, c.ts, reason)
        pending = []
        ...
        # protective exits (TP/SL): probe the bar's adverse extreme then the
        # favourable one so a stop/target hit by an intrabar WICK is captured —
        # a close-only check silently misses every intrabar trigger, which is why
        # the live bot (marking every ~10s) opens/closes far more than a bar-close
        # backtest. Reuses risk.on_market verbatim (long/short handling intact);
        # the first extreme that fires wins (conservative: adverse before
        # favourable). Falls back to the close mark for inventory trim.
        exit_evs: list = []
        pos = portfolio.positions.get(symbol)
        if pos is not None and pos.qty != 0:
            extremes = (c.low, c.high) if pos.qty > 0 else (c.high, c.low)
            for probe_px in extremes:
                probe = MarketEvent(symbol=symbol, price=probe_px,
                                    candles=candle_buf, ts=c.ts)
                out = risk.on_market(probe) or []
                if out:
                    exit_evs = out
                    break
        if not exit_evs:
            exit_evs = risk.on_market(me) or []  # close mark (inventory / none)
        for ev in exit_evs:
            if isinstance(ev, OrderEvent):
                pending.append((ev.order, "EXIT"))
        detector.on_market(me)
        # new entries from regime-appropriate strategy — queued for next-bar-open fill
        for sig in (manager.on_market(me) or []):
            for oe in (risk.on_signal(sig) or []):
                if isinstance(oe, OrderEvent):
                    pending.append((oe.order, "ENTRY"))
        ...
        if engine.halted:
            result.halted = True
    # orders generated on the final bar have no next bar to fill against — they
    # stay unfilled (open positions remain marked-to-market in the equity curve)
    # rather than fabricating a same-bar fill.
