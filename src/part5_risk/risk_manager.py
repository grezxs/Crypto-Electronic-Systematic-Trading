...
class RiskManager:
    def __init__(
        ...
        # daily-loss limit state (tier 2): pause NEW entries for the rest of the
        # calendar day, auto-reset on day rollover.
        self._day_key: Optional[int] = None
        self._day_start_equity: float = portfolio.equity
        self._day_blocked: bool = False
        self._soft_active: bool = False  # latched log flag for the soft tier
    ...
    # --- tier 1: soft drawdown de-risk ---
    def _dd_size_mult(self) -> float:
        """Soft tier: once drawdown exceeds ``drawdown_soft_pct`` (but is still
        below the hard halt) scale NEW-order size down by ``drawdown_soft_size_mult``
        to de-risk before the hard stop. Returns the sizing multiplier (1.0 = off)."""
        soft = float(self._p("drawdown_soft_pct", 0.0) or 0.0)
        if soft <= 0 or self._peak_equity <= 0:
            return 1.0
        dd = (self._peak_equity - self.portfolio.equity) / self._peak_equity
        if dd >= soft:
            if not self._soft_active:
                log.warning("SOFT DRAWDOWN %.1f%% (>= %.1f%%) — de-risking new orders to %.0f%% size",
                            dd * 100, soft * 100, float(self._p("drawdown_soft_size_mult", 0.5)) * 100)
                self._soft_active = True
            return float(self._p("drawdown_soft_size_mult", 0.5) or 0.5)
        self._soft_active = False
        return 1.0

    # --- tier 2: daily-loss limit (pause entries for the day) ---
    def _update_daily(self, ts: float) -> bool:
        """Pause NEW entries for the rest of the calendar day if the day's P&L
        breaches ``-daily_max_loss_pct`` of the day's starting equity. Auto-resets
        on day rollover. Protective exits (``on_market``) are unaffected, so open
        positions remain protectable. Returns True when entries are paused."""
        limit = float(self._p("daily_max_loss_pct", 0.0) or 0.0)
        if limit <= 0:
            return False
        day = int(ts // 86400) if ts else None
        ...
        if base > 0 and (self.portfolio.equity - base) / base <= -limit:
            log.warning("DAILY LOSS LIMIT %.1f%% hit (day P&L %.2f%%) — pausing entries until next day",
                        limit * 100, (self.portfolio.equity - base) / base * 100)
    ...
    def on_signal(self, event: SignalEvent) -> Optional[list[OrderEvent]]:
        ...
        # tier 2: daily-loss limit pauses NEW entries (exits still allowed)
        if self._update_daily(event.ts):
            return None
        ...
        qty = self._size(price) * self._dd_size_mult()  # tier 1: soft de-risk
        if qty <= 0:
            return None
