...
def _launch_dashboard() -> "subprocess.Popen | None":
    """Spawn the local web-terminal server (static ``frontend/`` + the bridge
    API that reads runtime/state.json and writes runtime/control.json) so a
    single ``python main.py`` brings up BOTH the bot and the monitor (one
    command, one Ctrl-C stops everything). Disable with ``DASHBOARD=0``.
    Returns the child process handle, or None if disabled.
    """
    if os.environ.get("DASHBOARD", "1") == "0":
        return None
    here = os.path.dirname(os.path.abspath(__file__))
    server = os.path.join(here, "scripts", "serve_frontend.py")
    port = os.environ.get("FRONTEND_PORT", "8000")
    try:
        proc = subprocess.Popen([sys.executable, server, "--port", str(port)])
        log.info("web terminal launching -> http://localhost:%s "
                 "(set DASHBOARD=0 to disable)", port)
        return proc
    except Exception:
        log.warning("failed to launch web terminal", exc_info=True)
        return None
...
def _stop_dashboard(proc: "subprocess.Popen | None") -> None:
    if proc is None:
        return
    log.info("stopping web terminal")
    proc.terminate()
    try:
        proc.wait(timeout=5)
    except Exception:
